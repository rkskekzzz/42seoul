...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/05/22 17:36:40 by suhshin           #+#    #+#             */
/*   Updated: 2021/05/22 17:38:56 by suhshin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_t(t_cylinder *cy, t_ray *ray, double t)
{
	...
	p = ray_at(ray, t);
	if (vec_dot(cy->n, vec_cal((t_vec[2]){p, cy->top},
					(double[2]){1, -1}, 2)) > 0)
		return (ERROR);
	if (vec_dot(cy->n, vec_cal((t_vec[2]){p, cy->bot},
					(double[2]){1, -1}, 2)) < 0)
		return (ERROR);
	return (OK);
}

int			get_cylinder_t(t_cylinder *cy, t_ray *ray,
