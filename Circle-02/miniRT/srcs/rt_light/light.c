...
int		in_shadow(t_minirt *mini, t_ray *light_ray, t_light *light,  t_hit_record *r)
{
	t_hit_record rec;
	double light_len;

	light_ray->dir = vec_cal((t_vec[2]){light->origin, r->p}, (double[2]){1, -1}, 2);
	light_ray->origin = vec_cal((t_vec[2]){r->p, r->n}, (double[2]){1, EPSILON}, 2);
...
t_clr	light_get(t_minirt *mini, t_hit_record *rec, t_light *light)
{
	...
	dir[3] = reflect(vec_oppo(dir[0]), rec->n);
